...
def vanity_projects
  ...
  # starring actor. Show the movie id and title and director's name.
...
def starring(whazzername)
  ...
  Movie.joins(:actors).where("actors.name LIKE '%#{whazzername.split("").join("%")}%' ").select("*")
end
...
def bad_years
  ...
  Movies.select(:yr).where("score = 8")
end
...
def longest_career
	#Find the actor and list all the movies of the actor who had the longest
  # career (the greatest time between first and last movie).
